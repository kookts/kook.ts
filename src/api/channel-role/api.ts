...
import {
  ...
  ChannelRolePermissionsFactory,
  KChannelRolePermissions,
...
export class ChannelRoleAPI extends ApiBase {
   ...
   */
  async index(channelId: string): Promise<Required<ChannelRolePermissions>> {
    const data = (
      await this.client.get(
        'v3/channel-role/index',
        this.toParams({
          channelId,
        })
      )
   ...
   * @param channelId 频道id
   */
  async sync(channelId: string): Promise<Required<ChannelRolePermissions>> {
    const data = (
      await this.client.post(
        'v3/channel-role/sync',
        this.toParams({
          channelId,
        })
      )
